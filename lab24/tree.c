...
//modded
Tree tree_create(Vector *tokens, int idx_left, int idx_right) //Token tokens[]
{
    ...
    for (int i = idx_left; i <= idx_right; ++i) {
        if ((v_get(tokens, i).type == BRACKET) && (v_get(tokens, i).data.is_left_bracket)) { //((tokens[i].type == BRACKET) && (tokens[i].data.is_left_bracket))
            ++brackets;
            continue;
        }
        if ((v_get(tokens, i).type == BRACKET) && !(v_get(tokens, i).data.is_left_bracket)) { //((tokens[i].type == BRACKET) && !(tokens[i].data.is_left_bracket)) {
        ...
        if (v_get(tokens, i).type == OPERATOR) { //(tokens[i].type == OPERATOR) {
            priority = get_priority(v_get(tokens, i).data.operator_name); //priority = get_priority(tokens[i].data.operator_name);
    ...
    if ((priority_min == get_priority('a')) &&
            (v_get(tokens, idx_left).type == BRACKET) && 
            (v_get(tokens, idx_left).data.is_left_bracket) &&
            (v_get(tokens, idx_right).type  == BRACKET) && 
            !(v_get(tokens, idx_right).data.is_left_bracket)) {
        free(t);
    ...
    if (v_get(tokens, op_pos).data.operator_name == '^') { //(tokens[op_pos].data.operator_name == '^') {
        // Поэтому ищем самый левый оператор степени, связанный с текущим
        brackets = 0;
        for (int i = op_pos; i >= idx_left; --i) {
            if ((v_get(tokens, i).type == BRACKET) && !(v_get(tokens, i).data.is_left_bracket)) {        /////
                ++brackets;
                continue;
            }
            if ((v_get(tokens, i).type == BRACKET) && (v_get(tokens, i).data.is_left_bracket)) {          //////
            ...
            if (v_get(tokens, i).type == OPERATOR) {                                             /////
                priority = get_priority(v_get(tokens, i).data.operator_name);                       //////
                if (priority == 3) {
                    op_pos = i;
                }
            }
    ...
    t->node  = v_get(tokens, op_pos); // Запись оператора     ///t->node  = tokens[op_pos];
